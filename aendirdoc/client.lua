...
RegisterCommand("doktor", function(source, args, raw)
	if (QBCore.Functions.GetPlayerData().metadata["isdead"]) or (QBCore.Functions.GetPlayerData().metadata["inlaststand"]) and spam then
		QBCore.Functions.TriggerCallback('aendirdoc:docOnline', function(EMSOnline, hasEnoughMoney)
			if EMSOnline <= Config.Doctor and hasEnoughMoney and spam then
				SpawnVehicle(GetEntityCoords(PlayerPedId()))
				TriggerServerEvent('aendirdoc:charge')
				Notify("Doktor Geliyor")
			else
				if EMSOnline > Config.Doctor then
					Notify("Aktif Doktor Var", "error")
				elseif not hasEnoughMoney then
					Notify("Paran Yok", "error")
				else
					Notify("Doktor Geliyor Yolda", "primary")
				end	
			end
		end)
	else
		Notify("Bunu yapabilmen için ölmen gerekiyor", "error")
	end
end)
...
function SpawnVehicle(x, y, z)  
	...
	if not DoesEntityExist(vehhash) then
		...
		SetVehicleNumberPlateText(mechVeh, "aendirdoc")
...
function DoctorNPC()
	...
	QBCore.Functions.Progressbar("revive_doc", "Doktor size tıbbi yardım yapıyor", Config.ReviveTime, false, false, {
		...
		Notify("Tedaviniz Yapıldı, Kesilen Ücret: "..Config.Price, "success")
